...
if [ -f "$BUILD/.config" ]; then
	for opt in OBJTOOL STACK_VALIDATION MODULE_SIG MODULE_SIG_ALL \
		...
		SECURITY_SELINUX X86_KERNEL_IBT CALL_THUNKS \
		MITIGATION_RETPOLINE MITIGATION_RETHUNK MITIGATION_UNRET_ENTRY \
		MITIGATION_CALL_DEPTH_TRACKING MITIGATION_RETBLEED MITIGATION_SRSO \
		MITIGATION_IBPB_ENTRY MITIGATION_IBRS_ENTRY JUMP_LABEL; do
